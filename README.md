...
4. Run partypatcher.exe to create the new, patched THPS3.exe game executable. This will be used to launch the game from now on.
5. Optionally (highly recommended), configure the game with partyconfig.exe
6. Launch the game from THPS3.exe

NOTE: if the game is installed in the "Program Files" directory, you may need to run each program as administrator. 
Also, if the game is installed in the "Program Files" directory, save files will be saved in the C:\Users\<name>\AppData\Local\VirtualStore directory.  
...
## Building
The build requires Cargo, Rust (i686-pc-windows-msvc target), and CMake (for the SDL from-scratch build). The project can be built with the following command:
```
cargo build --release
```
The project can also be built from Linux using [xwin](https://github.com/Jake-Shadle/xwin) with the following command:
```
cargo xwin build --release --xwin-arch x86
```
